...
const Links = ({title, fontSize, position, headerLinkHover, setHeaderLinkHover }) =>{
    ...
    return(
        <Link to={`/${title}`}>
            <div className={links.links_container} data-scroll data-scroll-speed='.25'
                 onMouseOver={e => linkOver(e)}
                 onMouseLeave={e => linkLeave(e)}>
                    <div className={`${links.link} + ${position}`}>
                        <h1 className={style.h1} style={{fontSize: `${fontSize}`}} >{title}</h1>
                        <svg viewBox="0 0 200 100"
                             fill="none"
                             className={links.arrow}
                             style={{transform: `${headerLinkHover ? 'translate(10%)' : 'translate(0%)' }`}}>
