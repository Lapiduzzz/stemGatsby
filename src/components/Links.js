...
const Links = ({link = '', title, fontSize, position, arrowSize, linkRef, arrRef,}) => {
    ...
    const {setImageCard} = useImageCard()

    const linkOver = (e) => {
        if (headerLinkHover === false) {
            setHeaderLinkHover(true)
            setImageCard(e.target.outerText)
        }
        if (arrowLoaded !== true) {
            setArrowLoaded(true)
        }
                ...
                <div className={`${links.link} + ${position}`}>
                    ...
                    <div className={links.arrow_container}>
                        <svg viewBox="0 0 200 100"
                            ...
                            <path d="M113.565 109C113.565 79.0915 137.965 54.5 167.641 54.5" stroke="#D9C8A9"
                                  stroke-width="5" stroke-miterlimit="10"/>
                            <path d="M167.641 54.5C137.965 54.5 113.565 29.9085 113.565 -1.47803e-06" stroke="#D9C8A9"
                                  stroke-width="5" stroke-miterlimit="10"/>
                        </svg>
                    </div>
                </div>
